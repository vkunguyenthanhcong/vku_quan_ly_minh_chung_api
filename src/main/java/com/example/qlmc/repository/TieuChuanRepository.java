...
public interface TieuChuanRepository extends JpaRepository<TieuChuan, Integer> {
    ...
    @Query (value = "SELECT tc.id_tieuchuan, tc.ten_tieuchuan, (SELECT COUNT(*) FROM minhchung mc WHERE mc.id_tieuchuan = tc.id_tieuchuan) total FROM tieuchuan tc WHERE tc.ma_ctdt = :maCtdt", nativeQuery = true)
